...
# 3. The path to the input folder
#
# Outputs:
...
REF_EPI_PATH=$(find $REF_FOLDER_PATH/EPIs -name "*_mc_mean.nii.gz")
REF_SEG_PATH=$(find $REF_FOLDER_PATH -name "*sc_seg.nii.gz")
REF_MASK_PATH=$(find $REF_FOLDER_PATH -name "*sc_mask.nii.gz")
INPUT_EPI_PATH=$(find $INPUT_FOLDER_PATH/EPIs -name "*_mc_mean.nii.gz")
EPI_REG_TO_REF=$INPUT_FOLDER_PATH/EPIs/EPI_reg_to_REF.nii.gz
